<template>
  <div
    class="mb-2 leading-5 shadow-raised card-button-style card-button-active"
    @mouseover="activeEdit = true"
    @mouseleave="activeEdit = false">
    <div class="relative flex px-3 pt-2 pb-1">
      <button
        v-if="!activeInput"
        class="inline-block w-full mb-1 text-left"
        @click="openPopup">
        {{ props.card.title }}
      </button>
      <button
        v-if="!activeInput"
        @click="isActiveInput"
        :class="{ block: activeEdit, hidden: !activeEdit }"
      ...
      <input
        ...
        @keydown.esc="isActiveInput"
        @keydown.enter="isActiveInput"
        type="text"
        class="mb-1" />
    </div>
  </div>
