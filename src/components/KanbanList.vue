<template>
  <div class="shadow-raised list-total-style list-width">
    <div class="list-header-style">
      <div class="flex">
        <div class="flex items-center grow">
          <h2
            v-if="showListHeader"
            @click="isShowListHeader"
            class="w-[214px] pl-3 pr-2 py-1.5 truncate">
            {{ props.List }}
          </h2>
          <input
            v-else
            :value="props.List"
            @keydown.esc="isShowListHeader"
            @keydown.enter="isShowListHeader"
            @input="handleInput"
            :class="{ focus: activeInput }"
    ...
    <div class="flex border-none list-footer-style">
      <button
        v-if="!activeCardPopup"
        @click="isActiveCardPopup"
      ...
      <div v-else class="add-card-popup-style">
        <input
          v-model="cardTitle"
          @keydown.enter="addCardByEnterKey"
          type="text"
